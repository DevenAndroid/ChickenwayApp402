...
Future getShippingList() async {
  if (shippingMethodsList.data == null) {
    await Repositories().postApi(url: ApiUrls.shippingMethodsUrl).then((value) {
      shippingMethodsList = ModelShippingMethodsList.fromJson(jsonDecode(value));
...
class _CheckoutCScreenState extends State<CheckoutCScreen> {
  ...
  createOrder() {
    ...
    map["phone_number"] = cartController.deliveryAddress.value.countryCode! + cartController.deliveryAddress.value.phoneNo!;
    ...
    cartController.repositories.postApi(url: ApiUrls.createOrderUrl, mapData: map, context: context).then((value) {
    ...
    SchedulerBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      if (addressController.model.value.data != null) {
        if (addressController.model.value.data!.userAddress!.isNotEmpty) {
          cartController.deliveryAddress.value = addressController.model.value.data!.userAddress!.first;
          if (shippingMethodsList.data != null) {
            ShippingData item = shippingMethodsList.data!.firstWhere(
                (element) => element.zoneName.toString() == cartController.deliveryAddress.value.shipping_city.toString(),
                orElse: () => ShippingData());
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: commonAppBar(title: "CHECKOUT", showCart: false, backGround: Colors.white),
        body: Obx(() {
            ...
            if (mapController != null) {
              if (cartController.allowNavigation == false) {
                ...
                LatLng ll = LatLng(double.parse(cartController.deliveryAddress.value.lat ?? "33.888630"),
                    ...
                    .checkLatLong;
                mapController!.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(target: ll, zoom: 15)));
                  ...
                  child: Column(children: [
                        ...
                        children: [
                            ...
                            children: [
                              Container(
                                ...
                                decoration: BoxDecoration(
                                    border: Border.all(color: const Color(0xffEAEAEA)), shape: BoxShape.circle),
                                child: const Icon(
                                  Icons.location_on,
                              ...
                              Expanded(
                                child: cartController.deliveryAddress.value.home != null
                                    ? Text(
                                        ...
                                        style: GoogleFonts.poppins(
                                            color: const Color(0xFF333333), fontSize: 14, fontWeight: FontWeight.w600),
                                      )
                                    : Text(
                              ...
                              InkWell(
                                ...
                                child: Padding(
                                  ...
                                  child: Text(
                                    cartController.deliveryAddress.value.home != null ? 'Change' : "Select",
                                    style: GoogleFonts.poppins(
                                        color: const Color(0xFFE02020), fontSize: 14, fontWeight: FontWeight.w500),
                                  ),
                                ),
                              )
                            ],
                          ...
                          Padding(
                            ...
                            child: Text(
                              ...
                              style: GoogleFonts.poppins(
                                  color: const Color(0xFF666666), fontSize: 13, fontWeight: FontWeight.w500),
                            ),
                          ),
                        ],
                      ),
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                          ...
                          boxShadow: const [BoxShadow(color: Colors.black26, blurRadius: 1, offset: Offset(.5, .5))]),
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.start,
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          ...
                          Row(
                            ...
                            children: [
                              Checkbox(
                                shape: const CircleBorder(side: BorderSide(color: Color(0xFFE02020))),
                    ...
                    if (cartController.selectedShippingMethod != null)
                      buildHero(size,
                          deliveryFee: cartController.selectedShippingMethod != null
                              ? (cartController.selectedShippingMethod!.shippingAmount!.isEmpty
                                  ? "Free"
                                  : cartController.selectedShippingMethod!.shippingAmount! +
                                      cartController.model.value.data!.cartmeta!.currencySymbol!)
                              : "Shipping Unavailable",
                          shippingAmount: int.tryParse(cartController.selectedShippingMethod!.shippingAmount!.toString())),
                    appBottomLogo()
                  ]),
                ),
              ),
              ...
              ? modelSwitchOff.value.data!.switchOffStore == 'Show'
                  ? Container(
                      ...
                      decoration: const BoxDecoration(
                          ...
                          boxShadow: [BoxShadow(color: Colors.black26, blurRadius: 1, offset: Offset(1, 1))]),
                      child: Padding(
                        padding: const EdgeInsets.only(top: 10, right: 15, left: 15, bottom: 16),
                        child: InkWell(
                          onTap: () {
