...
class _SingleProductScreenState extends State<SingleProductScreen> {
  ...
  Future getProductDetails() async {
    await repositories.postApi(
        url: ApiUrls.getSingleProductUrl,
        mapData: {"product_id": productId}).then((value) {
      ...
      if (model.value.productOption != null) {
        ...
        model.value.productOption!.removeWhere((element) =>
            befores
                .map((e) => e.uniId!.first.toString())
                .toList()
                .contains(element.uniId!.first.toString()) ||
            afters
                .map((e) => e.uniId!.first.toString())
                .toList()
                .contains(element.uniId!.first.toString()));
  ...
  Future updateCart({required bool increase}) async {
    ...
    if (model.value.productOption != null &&
      ...
      for (var item in model.value.selectedRadios.entries) {
        ...
        radioPrice
            .add((int.tryParse(item.value.toString().split("--").last) ?? 0));
      }
      ...
      if (model.value.productOption != null &&
        ...
        for (var i = 0; i < model.value.selected1.length; i++) {
          optionValues["tmcp_checkbox_1_$i"] =
              model.value.selected1[i].toString().split("--").first;
          optionPrice["$i"] = model.value.selected1[i]
              .toString()
              .split("--")
              .last
              .split("==========")
    ...
    if (optionValues.isNotEmpty) {
      ...
      map["option_price"] = optionPrice.entries
    ...
    await repositories
        .postApi(context: context, url: ApiUrls.updateCartUrl, mapData: map)
        .then((value) {
      ModelResponseCommon responseCommon =
            ...
            child: SingleChildScrollView(
              ...
              if (model.value.data != null) ...[
                if (model.value.productOption != null &&
                  ...
                  //   ...titlesUi(size),
                  if (model.value.productOption!.first
                        ...
                        children: [
                          Row(
                            children: [
                              Padding(
                                  padding:
                                      const EdgeInsets.all(8).copyWith(top: 10),
                                  child: buildShimmer(
                                    border: 15,
              ...
              child: Container(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      ...
                      bool canCheckOut = !validationCheck.entries
                          .map((e) => e.value)
                          .toList()
                          .contains(false);
                      if (canCheckOut) {
                        ...
                        showToast(
                            "Please select the above required addons to add to cart");
                      }
                      setState(() {});
                    },
  ...
  radioButtons(Size size, {required AddonsData addon}) {
    List<String> titles = addon.radiobuttonsHeaderTitle!.entries
        .map((e) => e.value.toString())
        .toList();
    List<String> subtitle = addon.radiobuttonsHeaderSubtitle!.entries
        .map((e) => e.value.toString())
        .toList();
    List<bool> required = addon.radioButtonsRequired!.entries
        .map((e) => e.value.toString() == "1")
        .toList();

    List<List<String>> radioValue =
        addon.radiobuttonsOptionsValue!.entries.map((e) => e.value).toList();
    List<List<String>> images =
        addon.radiobuttonsOptionsImage!.entries.map((e) => e.value).toList();
    List<List<String>> price =
        addon.radiobuttonsOptionsPrice!.entries.map((e) => e.value).toList();
    List<int> makeDefault = addon.radioOptionsDefaultValue!.entries
        .map((e) => int.tryParse(e.value.toString()) ?? -1)
        .toList();
    ...
    if (addon.makeDefaults) {
      for (var i = 0; i < makeDefault.length; i++) {
        if (makeDefault[i] != (-1)) {
          ...
          model.value.selectedRadios[
                  i.toString() + addon.uniId!.first.toString()] =
    ...
    return Column(
      children: List.generate(
          ...
          (index) => addonsRadio(
                ...
                title:
                    "${titles[index]}===$index${addon.uniId!.first.toString()}",
  ...
  titlesUi(Size size, {required AddonsData addon}) {
    ...
    List<String> titles =
        addon.title!.entries.map((e) => "${e.value}").toList();
    List<String> headerSubtitle =
        addon.headerSubtitle!.entries.map((e) => "${e.value}").toList();
  ...
  addonsCheckLists(Size size, {required AddonsData addon}) {
    List<List<String>> optionsTitle =
        addon.optionsTitle!.entries.map((e) => e.value).toList();
    // print(optionsTitle);
    addon.checkBoxesHeaderSubtitle!
    ...
    List<String> headerSubtitle = addon.checkBoxesHeaderSubtitle!.entries
        ...
        .toList();
    List<bool> required = addon.checkBoxesRequired!.entries
        .map((e) => e.value.toString() == "1")
        .toList();

    headerSubtitle.addAll(List.generate(
        20, (index) => "==========sd==${addon.uniId!.first.toString()}"));
    List<List<String>> optionsPrice =
        addon.optionsPrice!.entries.map((e) => e.value).toList();
    List<String> titles = addon.checkBoxesHeaderTitle!.entries
        .map((e) => e.value.toString())
        .toList();
    List<List<String>> images =
        addon.optionsImage!.entries.map((e) => e.value).toList();

    List<List<int>> makeDefault = addon.checkBoxesOptionsDefaultValue!.entries
        .map((e) =>
            e.value.map((e2) => (int.tryParse(e2.toString()) ?? -1)).toList())
        .toList();
    ...
    return Row(
      ...
      children: [
        Expanded(
            child: formatPrice2(
          ((double.tryParse(
                          (model.value.data!.first.price ?? "0").toString()) ??
                      0) +
                  (model.value.productOption!.isNotEmpty
                      ? (model.value.selected1
                              .map((e) =>
  ...
  Padding addonsRadio({
    ...
    bool showError2 = isRequired &&
        model.value.selectedRadios.entries
            .where((element) =>
                element.value.toString().split("||").last.split("--").first ==
                title)
            .toList()
            .isEmpty;
    if (showError2) {
              ...
              itemBuilder: (context, index) {
                return ListTile(
                  onTap: () {
                    ...
                    model.value.selectedRadios[indexTop.toString()] =
                        "${options[index]}||$title--${price[index]}";
                    setState(() {});
                    return;
                  },
                      ...
                      if (price[index].isNotEmpty && price[index] != '0')
                        formatPrice2(
                            ...
                            GoogleFonts.poppins(
                                ...
                                fontSize: 14,
                                fontWeight: FontWeight.w400)),
                      model.value.selectedRadios[indexTop] ==
                              "${options[index]}||$title--${price[index]}"
                          ? Icon(
                              Icons.check_circle,
                              color: darkRedColor,
  ...
  addonsCheckList(Size size,
    ...
    bool showError1 = (isRequired == true &&
        model.value.selected1
            .where((element) =>
                (element.toString().split("--")[1].toString()) ==
                "$goodIndex+$subTitle")
            .toList()
            .isEmpty);
            ...
            ListView.separated(
              ...
              itemBuilder: (context, index) {
                if (addon.makeDefaultsCheckBox) {
                  if (defaults[index] != (-1)) {
                    model.value.selected1.add(
                        "${optionTitles[index]}--$goodIndex+$subTitle--${optionPrice[index]}$subTitle");
                    assignedOptions++;
                    if (assignedOptions == allAvailableOptions ||
                        assignedOptions > allAvailableOptions) {
                      addon.makeDefaultsCheckBox = false;
                    }
                    model.value.selected1 =
                        model.value.selected1.toSet().toList();
                  }
                }
                return Row(
                  ...
                  children: [
                    ...
                    if (optionPrice[index].isNotEmpty &&
                        optionPrice[index] != '0')
                      formatPrice2(
                          optionPrice[index].isNotEmpty
                              ? optionPrice[index]
                              : "0",
                          model.value.data!.first.currencySymbol ?? '',
                          GoogleFonts.poppins(
                    ...
                    Theme(
                      data: ThemeData(
                          ...
                          primaryColor: darkRedColor,
                          unselectedWidgetColor: darkRedColor),
                      child: Checkbox(
                          activeColor: AppTheme.primaryColor,
                          shape: RoundedRectangleBorder(
                              borderRadius: BorderRadius.circular(3),
                              side: BorderSide(color: darkRedColor)),
                          value: model.value.selected1.contains(
                              "${optionTitles[index]}--$goodIndex+$subTitle--${optionPrice[index]}$subTitle"),
                          // value: true,
                          onChanged: (value) {
                            if (value == true) {
